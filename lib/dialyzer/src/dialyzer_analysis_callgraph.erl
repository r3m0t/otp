...
compile_and_store(Files, #analysis_state{codeserver = CServer,
  ...
  CServer = dialyzer_codeserver:set_next_core_label(NextLabel, CServer),
  ...
  Callgraph = cleanup_callgraph(State, CServer, Callgraph, Modules),
  {T3, _} = statistics(runtime),
  Msg2 = io_lib:format("done in ~.2f secs\n", [(T3-T2)/1000]),
  send_log(Parent, Msg2),
  {Callgraph, sets:from_list(NoWarn), CServer}.
...
store_core(Mod, Core, NoWarn, Callgraph, CServer) ->
  ...
  {ok, cerl_trees:size(CoreTree), {Mod, CoreTree, NoWarn, Callgraph}}.

-spec continue_compilation(integer(), compilation_data()) -> file_result().

continue_compilation(NextLabel, {Mod, CoreTree, NoWarn, Callgraph}) ->
  {LabeledTree, _NewNextLabel} = cerl_trees:label(CoreTree, NextLabel),
  LabeledCore = cerl:to_records(LabeledTree),
  store_code_and_build_callgraph(Mod, LabeledCore, Callgraph, NoWarn).
...
store_code_and_build_callgraph(Mod, Core, Callgraph, NoWarn) ->
  CoreTree = cerl:from_records(Core),
